...
Interlock's value depends on continuity without requiring the user to manually
...
any AI prompt. Interlock's supported redaction is defense in depth, not a
...
`interlock_interlock-postgres`. Model files live in
`interlock-memory_interlock-models`. Access tokens live in
`~/.config/interlock/` and generated authorization state lives under
`distribution/state/`.
...
archive is the complete portable export of stored Interlock data. Protect it as
...
state, delete only these Interlock paths:

```sh
rm -rf ~/.config/interlock
rm -rf distribution/state
rm -f distribution/.env
rm -f ~/.local/bin/interlock-mcp ~/.local/bin/interlock-gate
```
